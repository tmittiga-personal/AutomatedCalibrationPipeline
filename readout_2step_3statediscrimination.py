...
def readout_amplitude_binary_search(
    ...
    with program() as IQ_blobs:
        ...
        with stream_processing():
            ...
            I_g_st.buffer(2).save_all("I_g")
            Q_g_st.buffer(2).save_all("Q_g")
            I_e_st.buffer(2).save_all("I_e")
            Q_e_st.buffer(2).save_all("Q_e")
            I_f_st.buffer(2).save_all("I_f")
            Q_f_st.buffer(2).save_all("Q_f")
    ...
    # For ge resonator
    # g vs e
    anglege, thresholdge, fidelityge, gg, ge, eg, ee_g, figge = two_state_discriminator_plot(Ig[1], Qg[1], Ie[1], Qe[1], b_print=True)
    # g vs f
    anglegf_g, thresholdgf_g, fidelitygf_g, gg_g, gf_gg, fg_g, ff_g, figgf_g = two_state_discriminator_plot(Ig[1], Qg[1], If[1], Qf[1], b_print=True)
    # For ef resonator
    # g vs f
    anglegf_f, thresholdfg_f, fidelitygf_f, gg_f, gf_f, fg_f, ff_g, figgf_f = two_state_discriminator_plot(Ig[0], Qg[0], If[0], Qf[0], b_print=True)
    # e vs f
    angle_ef, thresholdef, fidelityef, ee_f, ef, fe, ff_f, figef = two_state_discriminator_plot(Ie[0], Qe[0], If[0], Qf[0], b_print=True)
    ...
    IQ_blobs_data[f"figure_ge_measuredge"] = figge
    IQ_blobs_data[f"figure_ef_measuredef"] = figef
    IQ_blobs_data[f"figure_fg_measuredef"] = figgf_f
    IQ_blobs_data[f"figure_gf_measuredge"] = figgf_g

    results_dict[READOUT_TYPE] = {
        "fidelityge": fidelityge,
        "fidelitygf_measuredge": fidelitygf_g,
        "fidelityef": fidelityef,
        "fidelityfg_measuredef": fidelitygf_f,
        "anglege": np.mod(anglege + mc.RR_CONSTANTS[resonatorge]["rotation_angle"], 2*np.pi),  # Update angle
        "anglegf_measuredge": np.mod(anglegf_g + mc.RR_CONSTANTS[resonatorge]["rotation_angle"], 2*np.pi),  # Update angle
        "angleef": np.mod(angle_ef + mc.RR_CONSTANTS[resonator]["rotation_angle"], 2*np.pi),  # Update angle
        "anglefg_measuredef": np.mod(anglegf_f + mc.RR_CONSTANTS[resonator]["rotation_angle"], 2*np.pi),  # Update angle
        "thresholdge": thresholdge,
        "thresholdef": thresholdef,
        "thresholdgf_measuredge": thresholdgf_g,
        "thresholdfg_measuredef": thresholdfg_f,
        'amplitude': initial_amplitude*amplitude_scale,
    }
    plt.close()
    IQ_blobs_data['results_dict'] = results_dict
        
    data_folder = data_handler.save_data(IQ_blobs_data, name=f"{qubit}_2step_3statediscrimination")

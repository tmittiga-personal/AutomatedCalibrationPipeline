...
class parity_ramsey:
    ...
    def __init__(
        ...
        self.f_artificial = 0.1 * self.mc.u.MHz
    ...
    def fit_parity_beat(self):
        ...
        y = self.data_dict[resonator]['P_T2star']
        yerr = self.data_dict[resonator]['P_T2star_err']
        ysig = np.mean(yerr, axis=0)  # average errorbar to put into curve_fit sigma argument
        ...
        # Loop over possible frequencies
        best_fit_resq = 1e10
        f_seeds = [[i,j] for i in np.arange(-0.5,0.6,0.5) for j in np.arange(-0.5,0.6,0.5)]

        # fit function. Correlate amplitudes of two frequency components
        ...
        for f_seed in f_seeds:
            ...
            bounds = ([0,strongest_freqs[1]-1.5*df, 0.8*t2_bk_estimate, 500,  strongest_freqs[0]-1.5*df],
                        [1.2*t2_amp_estimate,strongest_freqs[1]+1.5*df, 1.2*t2_bk_estimate, 2*taus[-1], strongest_freqs[0]+1.5*df])

            try:
                fit_paramsT, covar, infodict, mesg, ier = optimize.curve_fit(fitfunc, taus,y, p0=p0, full_output= True,
                    bounds=bounds, sigma=ysig) #, sigma=wd9e,
                ...
                print('Frequency seed failed. Try again')
                continue
        ...
        ax[i_pq, 0].errorbar(4 * np.array(self.taus), self.data_dict[resonator]['P_T2star'], yerr=self.data_dict[resonator]['P_T2star_err'], label=f'T2star',

...
class ef_t2star_t13_t1f:
    def __init__(
        ...
        self.f_artificial = 500 * self.mc.u.kHz
        ...
        self.taus = np.concatenate(
            (np.arange(4, 80_000//4 , 400//4), # 0.1 MHz  #np.arange(4, 200_000//4 , 500//4)
            )
        )
    ...
    def precomile_muliplexed_ramsey(self):
        ...
        for i_i in range(self.program_divisions):
            ...
            with program() as multiplex_ramsey: #_{i_i}:
                ...
                i_case = declare(int)  # QUA variable for applying T1 or T2*
                ...
                with for_(n, 0, n < self.n_avg, n + 1):        
                    with for_each_(t, self.tau_arrays[i_i]):  
    ...
    def run_ef_t2star_t13_t1f(self):       
        ...
        while self.qm.queue.count > 0 or job is not None: 
            ...
            while results.is_processing():
                ...
                ax[2*i_pq, 0].errorbar(4 * self.tau_arrays[i_job], I[0], label=f'I_T1_|e>',
                                            ecolor='r', fmt='.', elinewidth=1, capsize=None)
                ax[2*i_pq, 0].errorbar(4 * self.tau_arrays[i_job], I[1], label=f'I_T2*',
                                            ecolor='b', fmt='.', elinewidth=1, capsize=None)
                ax[2*i_pq, 0].errorbar(4 * self.tau_arrays[i_job], I[2], label=f'I_T1_|f>',
                ...
                ax[2*i_pq+1, 0].errorbar(4 * self.tau_arrays[i_job], Q[0], label=f'Q_T1',
                                            ecolor='r', fmt='.', elinewidth=1, capsize=None)
                ax[2*i_pq+1, 0].errorbar(4 * self.tau_arrays[i_job], Q[1], label=f'Q_T2*',
                                            ecolor='b', fmt='.', elinewidth=1, capsize=None)
                ax[2*i_pq+1, 0].errorbar(4 * self.tau_arrays[i_job], Q[2], label=f'Q_T1_|e>',
                                            ecolor='k', fmt='.', elinewidth=1, capsize=None)
        ...
        for i_pd, data in enumerate(self.program_data_dict.values()):
            ...
            if i_pd == 0:
                I_T1e = np.array(data[resonator]['I_T1e'])
                Q_T1e = np.array(data[resonator]['Q_T1e'])
                I_T2star = np.array(data[resonator]['I_T2star'])
                Q_T2star = np.array(data[resonator]['Q_T2star'])
                I_T1f = np.array(data[resonator]['I_T1f'])
                Q_T1f = np.array(data[resonator]['Q_T1f'])
            else:
                I_T1e = np.concatenate((I,np.array(data[resonator]['I_T1e'])))
                Q_T1e = np.concatenate((Q,np.array(data[resonator]['Q_T1e'])))
                I_T2star = np.concatenate((I,np.array(data[resonator]['I_T2star'])))
                Q_T2star = np.concatenate((Q,np.array(data[resonator]['Q_T2star'])))
                I_T1f = np.concatenate((I,np.array(data[resonator]['I_T1f'])))
                Q_T1f = np.concatenate((Q,np.array(data[resonator]['Q_T1f'])))
        ...
        ax[2*i_pq, 0].errorbar(4 * np.array(self.taus), self.data_dict[resonator]['I_T2star'], label=f'I_T2*',
                                    ecolor='b', fmt='.', elinewidth=1, capsize=None)
        ax[2*i_pq, 0].errorbar(4 * np.array(self.taus), self.data_dict[resonator]['I_T1e'], label=f'I_T1_|e>',
                                    ecolor='b', fmt='.', elinewidth=1, capsize=None)
        ax[2*i_pq, 0].errorbar(4 * np.array(self.taus), self.data_dict[resonator]['I_T1f'], label=f'I_T1_|f>',
                                    ecolor='b', fmt='.', elinewidth=1, capsize=None)
        ax[2*i_pq, 0].set_xlabel('durations (ns)')
        ax[2*i_pq, 0].set_ylabel("I Voltage")
        ax[2*i_pq, 0].legend()
        ...
        ax[2*i_pq+1, 0].errorbar(4 * np.array(self.taus), self.data_dict[resonator]['Q_T2star'], label=f'Q_T2*',
                                    ecolor='b', fmt='.', elinewidth=1, capsize=None)
        ax[2*i_pq+1, 0].errorbar(4 * np.array(self.taus), self.data_dict[resonator]['Q_T1e'], label=f'Q_T1_|e>',
                                    ecolor='b', fmt='.', elinewidth=1, capsize=None)
        ax[2*i_pq+1, 0].errorbar(4 * np.array(self.taus), self.data_dict[resonator]['Q_T1f'], label=f'Q_T1_|f>',
                                    ecolor='b', fmt='.', elinewidth=1, capsize=None)
        ax[2*i_pq+1, 0].set_xlabel('durations (ns)')
        ax[2*i_pq+1, 0].set_ylabel("Q Voltage")
        ax[2*i_pq+1, 0].legend()
        ...
        self.data_folder = self.data_handler.save_data(self.multiplex_ramsey_data, name="e_f_t2star_t1e_t1f")
    ...
    def fit_parity_beat(self):
        ...
        It1e = self.data_dict[resonator]['I_T1e']
        It1f = self.data_dict[resonator]['I_T1f']
        y = It2 - (It1e+It1f)/2

...
class Ramsey_w_virtual_rotation:
    def __init__(
        ...
        self.taus = taus
        self.wait = 75_000 # in clock cycles = 4ns
    ...
    def ramsey_w_virtual_rotation(
        ...
        with program() as ramsey:
            ...
            with for_(n, 0, n < n_avg, n + 1):
                with for_(*from_array(tau, taus)): #for_each_(tau, taus): #for_(*from_array(tau, taus)):
                    ...
                    ##############################
                    play("cw", self.resonatorQP)
                    wait(self.wait-tau, self.resonatorQP)
                    align()
                    # 1st x90 gate
                    play("x180", self.qubit)
                    # Wait a varying idle time
        ...
        if simulate:
            ...
            while results.is_processing():
                ...
                plt.subplot(311)
                plt.cla()
                plt.plot(4 * taus, I[0], ".", label = f'{self.wait*4}ns Wait')
                plt.plot(4 * taus, I[1], ".", label = f'{self.wait*4}ns Fixed')
                plt.ylabel("I quadrature [V]")
                plt.subplot(312)
                plt.cla()
                plt.plot(4 * taus, Q[0], ".", label = f'{self.wait*4}ns Wait')
                plt.plot(4 * taus, Q[1], ".", label = f'{self.wait*4}ns Fixed')
                plt.ylabel("Q quadrature [V]")
                plt.subplot(313)
                plt.cla()
                plt.plot(4 * taus, state[0], ".", label = f'{self.wait*4}ns Wait')
                plt.plot(4 * taus, state[1], ".", label = f'{self.wait*4}ns Fixed')
                plt.ylim((0, 1))

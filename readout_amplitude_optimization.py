...
def readout_amplitude_optimization(
    qubit,
    resonator,
):
    ...
    original_amplitude = RR_CONSTANTS[resonator]["amplitude"]
    ...
    if simulate:
        ...
        # Plot the data
        fig = plt.figure()
        amplitude_sweep = amplitudes * original_amplitude
        plt.plot(amplitude_sweep, fidelity_vec, "b.-", label="averaged fidelity")
        plt.plot(amplitude_sweep, ground_fidelity_vec, "r.-", label="ground fidelity")
        plt.title("Readout amplitude optimization")
        plt.xlabel("Readout amplitude [V]")
        plt.ylabel("Readout fidelity [%]")
        plt.legend(
            (
                f"readout_amp = {original_amplitude * amplitudes[np.argmax(fidelity_vec)] / u.mV:.3f} mV, for {max(fidelity_vec):.1f}% averaged fidelity",
                f"readout_amp = {original_amplitude * amplitudes[np.argmax(ground_fidelity_vec)] / u.mV:.3f} mV, for {max(ground_fidelity_vec):.1f}% ground fidelity",
            )
        )

        ro_amp_optimization["figure"] = fig
        ro_amp_optimization["amplitude_sweep"] = amplitude_sweep
        ro_amp_optimization["averaged_optimal_amplitude"] = original_amplitude * amplitudes[np.argmax(fidelity_vec)]
        ro_amp_optimization["ground_optimal_amplitude"] = original_amplitude * amplitudes[np.argmax(ground_fidelity_vec)]

        data_folder = data_handler.save_data(ro_amp_optimization, name=f"{resonator}_ro_amp_optimization")
